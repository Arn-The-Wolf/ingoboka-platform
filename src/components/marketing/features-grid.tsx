        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 lg:gap-8">
          {FEATURES.map(({ icon: Icon, titleKey, bodyKey }) => (
            <article
              key={titleKey}
              className={cn(
                'flex flex-col gap-4 rounded-2xl border border-brand-border/60 bg-white p-6',
                'transition-shadow hover:shadow-elevated'
              )}
            >
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-brand-primary-light">
